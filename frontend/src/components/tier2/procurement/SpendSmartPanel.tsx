...
import { PieChart, Upload, AlertTriangle , Settings} from 'lucide-react'
import POCConfigManager from '../../POCConfigManager'
import FileUpload from '../../FileUpload'

interface SpendSmartPanelResponse {
  results: any
  insights: string[]
  recommendations?: string[]
}

export default function SpendSmartPanel() {
  const [showConfig, setShowConfig] = useState(false)
  const [textInput, setTextInput] = useState('')
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<SpendSmartPanelResponse | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post<SpendSmartPanelResponse>(
        'http://localhost:8000/api/v1/modules/spend-smart/analyze',
        { text: textInput },
...
<div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Procurement spend analysis and optimization</p>
      </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <p className="text-sm text-slate-600 mb-4">
          Upload purchase orders, invoices, or spend reports for analysis.
        </p>
        <FileUpload
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Enter your spend analysis query
          </label>
          <textarea
            value={textInput}
            onChange={(e) => setTextInput(e.target.value)}
            placeholder="e.g., Analyze uploaded spend data for optimization opportunities..."
            rows={4}
