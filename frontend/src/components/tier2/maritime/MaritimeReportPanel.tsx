...
import { Ship, Upload, AlertTriangle , Settings} from 'lucide-react'
import POCConfigManager from '../../POCConfigManager'
import FileUpload from '../../FileUpload'

interface MaritimeReportPanelResponse {
  results: any
  insights: string[]
  recommendations?: string[]
}

export default function MaritimeReportPanel() {
  const [showConfig, setShowConfig] = useState(false)
  const [textInput, setTextInput] = useState('')
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<MaritimeReportPanelResponse | null>(null)
  const [error, setError] = useState<string | null>(null)

  const handleSubmit = async () => {
    if (!textInput.trim()) {
      setError('Please enter a maritime logistics query')
      return
    }
    ...
    try {
      const response = await axios.post<MaritimeReportPanelResponse>(
        'http://localhost:8000/api/v1/modules/maritime-logistics/analyze',
        { text: textInput },
...
<div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Generate maritime logistics and shipping reports</p>
      </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4">📋 Upload Maritime Documents</h2>
        <p className="text-sm text-gray-600 mb-4">
          Upload shipping manifests, port reports, or logistics documents.
        </p>
        <FileUpload
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <textarea
          ...
          placeholder="Enter your maritime logistics query..."
          rows={4}
