...
import { Scale, Upload, AlertTriangle , Settings} from 'lucide-react'
import POCConfigManager from '../../POCConfigManager'
import FileUpload from '../../FileUpload'

interface LegalDocumentPanelResponse {
  results: any
  insights: string[]
  recommendations?: string[]
}

export default function LegalDocumentPanel() {
  const [showConfig, setShowConfig] = useState(false)
  const [textInput, setTextInput] = useState('')
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<LegalDocumentPanelResponse | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post<LegalDocumentPanelResponse>(
        'http://localhost:8000/api/v1/modules/legal-document/analyze',
        { text: textInput },
...
<div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Analyze and extract insights from legal documents</p>
      </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <h3 className="text-lg font-semibold text-slate-800 mb-2">
          ⚖️ Upload Legal Documents
        </h3>
        <p className="text-sm text-slate-600 mb-4">
          Upload contracts, agreements, legal briefs, or case documents.
        </p>
        <FileUpload
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Enter your legal query or analysis request
          </label>
          <textarea
            value={textInput}
            onChange={(e) => setTextInput(e.target.value)}
            placeholder="e.g., Analyze uploaded contract for liability clauses..."
            rows={4}
