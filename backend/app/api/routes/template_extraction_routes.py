...
class AutoExtractRequest(BaseModel):
    """Request model for auto template selection based on URL"""
    url: str
    session_id: Optional[str] = None
        ...
        if preset_name == "screener_in":
            ...
            if not has_valid_selector:
                ...
                return ExtractionResponse(
                    ...
                    data=result.get("table", []),  # UltraSmartExtractor returns 'table', we map to 'data'
                    row_count=result.get("row_count", 0),
                    extracted_at=datetime.utcnow().isoformat(),
                    session_id=request.session_id,
                    error=result.get("error")
...
async def save_css_template(
    ...
    **Example Usage:**
    ...
    try:
        ...
        # Validate URL pattern is unique (prevent duplicate templates for same domain)
        if request.url_pattern:
            result = await db.execute(
                text("""
                    SELECT name, display_name, url_pattern
                    FROM saved_css_templates
                    WHERE url_pattern = :url_pattern AND is_active = TRUE
                """),
                {"url_pattern": request.url_pattern}
            )
            duplicate = result.fetchone()
            if duplicate:
                existing_name, existing_display_name, existing_pattern = duplicate
                raise HTTPException(
                    status_code=400,
                    detail={
                        "error": "Duplicate URL Pattern",
                        "message": f"A template with the same URL pattern already exists",
                        "existing_template": {
                            "name": existing_name,
                            "display_name": existing_display_name,
                            "url_pattern": existing_pattern
                        },
                        "suggestions": [
                            f"Use a more specific URL pattern (e.g., '{request.url_pattern}/category/*')",
                            f"Update the existing template '{existing_display_name}' instead",
                            "Use a different URL pattern that doesn't overlap"
                        ],
                        "note": "Multiple templates with identical patterns will return the same results"
                    }
                )
...
async def list_all_templates(
    filter_mode: Optional[str] = None,  # "css", "all", or None (default: all)
    db: AsyncSession = Depends(get_db)
):
    """
    **LIST ALL CSS TEMPLATES**
    ...
    Query Parameters:
    - filter_mode: Optional filter for extraction mode
      - "css": Show ONLY templates with CSS selectors (for CSS Extractor mode)
      - "all" or None: Show ALL templates (for Template Mapper mode)
    ...
    try:
        ...
        if filter_mode == "css":
            # Filter to show ONLY templates with CSS selectors
            all_templates = [t for t in all_templates if t.get("has_css_selectors", False)]
            logger.info(f"Filtered templates for CSS mode: {len(all_templates)} templates with CSS selectors")
        else:
            # Show all templates (default for Template Mapper mode)
            logger.info(f"Returning all templates: {len(all_templates)} total")

        return {
            "templates": all_templates,
            "total_count": len(all_templates),
            "builtin_count": len([t for t in all_templates if t.get("source") == "builtin"]),
            "saved_count": len([t for t in all_templates if t.get("source") == "user_saved"]),
